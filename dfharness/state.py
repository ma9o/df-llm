...
def compact_result(view, before):
    ...
    result = {
        k: deepcopy(view[k])
        for k in ("state_id", "effect_id", "status", "dispatch", "dispatch_replayed", "action")
        if k in view
    }
    ...
    changes: dict[str, object] = {
        ...
        "inventory_changed": [
            {"before": item_summary(old[key]), "after": item_summary(i)}
            for key, i in new.items()
            if key in old and item_summary(old[key]) != item_summary(i)
        ],
    }
    ...
    for field, initial, final in (
        (
            "position",
            before.get("status", {}).get("position"),
            view.get("status", {}).get("position"),
        ),
        (
            "health",
            (before.get("adventurer") or {}).get("health"),
            (view.get("adventurer") or {}).get("health"),
        ),
    ):
        if initial != final:
            changes[field] = {"before": initial, "after": final}
    old_units = {
        u["id"]: u
        for u in (before.get("map") or {}).get("units", [])
        if u["id"] != before.get("status", {}).get("adventurer_id")
    }
    new_units = {
        u["id"]: u
        for u in (view.get("map") or {}).get("units", [])
    ...
    changes.update(
        ...
        visible_units_changed=[
            {"before": deepcopy(old_units[i]), "after": deepcopy(u)}
            for i, u in new_units.items()
            if i in old_units and old_units[i] != u
        ],
    )
    ...
    if view.get("menu"):
        ...
        result["menu"]["options"] = [
            {
                k: o[k]
                for k in ("id", "item_id", "container_id", "kind", "label", "visible")
                if k in o
            }
    ...
    if view.get("status", {}).get("modal") or view.get("dispatch", {}).get("outcome") in (
        "needs_input",
        "failed",
        "no_effect",
    ):
