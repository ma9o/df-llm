...
def parser():
    p = argparse.ArgumentParser(description="Control a running Dwarf Fortress through DFHack; adventure mode first.")
    p.add_argument("--port", type=int, help="Override DFHACK_PORT and the game's remote-server.json")
    p.add_argument("--timeout", type=float, default=10, help="RPC response timeout in seconds")
    p.add_argument("--log", type=Path, help="Append requests and observations to a JSONL episode log")
    p.add_argument("--mode", choices=["step", "complete"], default="step", help="Controller's default dispatch mode")
    p.add_argument("--acknowledge", action=argparse.BooleanOptionalAction, default=False,
    ...
    p.add_argument("--interrupt-on", type=json.loads, default={}, help="Controller interruption conditions as a JSON object")
    subs = p.add_subparsers(dest="command", required=True)
    subs.add_parser("doctor", help="Find CrossOver, DFHack, saves, and the RPC connection")
    setup = subs.add_parser("setup", help="Set the game's local RPC port; restart DFHack after changing it")
    ...
    launcher.add_argument("--direct", action="store_true", help="Launch DF directly with its installed DFHack hook")
    status = subs.add_parser("status", help="Read all supported character state and current game/input status")
    status.add_argument("--text", action="store_true", help="Render the character report as readable text")
    subs.add_parser("game-status", help="Read only the mode, screen, turn readiness, and version")
    character = subs.add_parser("character-status", help="Read comprehensive character health, attributes, skills, needs, personality, and equipment")
    character.add_argument("--text", action="store_true", help="Render the character sheet as readable text")
    obs = subs.add_parser("observe", help="Read UI text, a local ASCII map, and structured adventure state")
    ...
    obs.add_argument("--text", action="store_true", help="Compact human/LLM-readable view instead of JSON")
    obs.add_argument("--radius", type=int, default=20, help="Nearby item search radius")
    items = subs.add_parser("items", help="Read visible nearby items and container contents in one call")
    ...
    interrupt = subs.add_parser("interrupt", help="Request that a running dispatch stop before further inputs")
    ...
    ready = subs.add_parser("wait-ready", help="Wait without advancing the game until it can accept input")
    ...
    actions["move"].add_argument("direction", choices=["n", "s", "e", "w", "ne", "nw", "se", "sw", "up", "down"])
    actions["wait"] = subs.add_parser("wait", help="Take one short in-game wait action")
    actions["dismiss"] = subs.add_parser("dismiss", help="Acknowledge one detected help or announcement page")
    actions["resume"] = subs.add_parser("resume", help="Apply dispatch policy to the current state without repeating prior input")
    actions["resume"].add_argument("dispatch_id", nargs="?", help="Resume this saved workflow, including across client processes")
    for name in ("pickup", "equip", "wield", "remove", "drop", "stow"):
        actions[name] = subs.add_parser(name, help=f"Dispatch {name} by item ID and verify its inventory result")
    ...
    for name in ("equip", "wield"):
        ...
        actions[name].add_argument("--disposition", choices=["hold", "drop", "stow"], default="hold")
    ...
    actions["walk-to"] = subs.add_parser("walk-to", help="Walk to a tile in the currently observed local area")
    for name in ("x", "y", "z"):
        actions["walk-to"].add_argument(name, type=int)
    actions["walk-to"].add_argument("--allow-occupied", action="store_true", help="Allow routes through occupied tiles")
    ...
    actions["select-option"] = subs.add_parser("select-option", help="Select a currently visible structured menu option")
    actions["select-option"].add_argument("option_id")
    actions["respond"] = subs.add_parser("respond", help="Respond to the current Continue/Stop/Finish prompt")
    actions["respond"].add_argument("choice", choices=["continue", "stop", "finish"])
    actions["select-unit"] = subs.add_parser("select-unit", help="Select a visible creature in the conversation picker")
    ...
    actions["choose"] = subs.add_parser("choose", help="Click a unique visible label in the text UI")
    actions["choose"].add_argument("label")
    actions["text"] = subs.add_parser("text", help="Type printable ASCII into the focused text field")
    ...
    for action in actions.values():
        ...
        action.add_argument("--mode", dest="dispatch_mode", choices=["step", "complete"], help="Override dispatch completion mode")
        action.add_argument("--acknowledge", dest="dispatch_acknowledge", action=argparse.BooleanOptionalAction,
                            default=None, help="Override automatic help/announcement acknowledgements")
        action.add_argument("--max-steps", dest="dispatch_max_steps", type=int, help="Override dispatch input limit")
        action.add_argument("--seconds", type=float, default=30, help="Dispatch time limit, including automatic responses")
        action.add_argument("--interrupt-on", dest="dispatch_interrupt_on", type=json.loads, help="Override controller interruption conditions")
...
def main(argv=None):
    ...
    defaults = {"mode": args.mode, "acknowledge": args.acknowledge, "max_steps": args.max_steps,
    ...
    try:
        if args.command == "setup":
            ...
            return subprocess.run([str(Path(__file__).resolve().parents[1] / "df-ascii")], env=env).returncode
        elif args.command == "run":
            if not args.args:
                raise ValueError("Usage: ./dfctl run COMMAND [ARG ...]")
            sys.stdout.write(run_command(*args.args, port=port_for_game(args.port), timeout=args.timeout))
            return 0
        else:
            ...
            if args.command == "status":
                ...
                center = dict(zip(("x", "y", "z"), args.center)) if args.center else None
                result = client.observe(args.width, args.height, center, not args.no_map, args.radius)
                ...
                if args.command == "act":
                    ...
                    for key in ("direction", "key", "x", "y", "z", "button", "unit_id", "item_id", "option_id",
                                "dispatch_id", "replace", "disposition", "container_id", "body_part_id",
                                "allow_occupied", "max_liquid_depth", "blocked_tiles"):
                ...
                execution = {key: getattr(args, "dispatch_" + key) for key in ("mode", "acknowledge", "max_steps", "interrupt_on")
                             if getattr(args, "dispatch_" + key) is not None}
                result = client.act(action, args.expect, args.request_id, args.seconds, execution, args.result_format)
        ...
        print("Interrupted. An in-flight action may have executed; observe before retrying.", file=sys.stderr)
        return 130
