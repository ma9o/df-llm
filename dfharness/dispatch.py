...
def execution_policy(defaults=None, override=None):
    ...
    if not isinstance(rules, dict) or rules.keys() - {
        "blood_loss",
        "new_wounds",
        "new_visible_units",
        "visible_unit_ids",
...
def interruption_reason(before, view, events, policy):
    ...
    for condition, field, compare in (
        ...
        if (
            ...
            and field in old
            and field in new
            and compare(old[field], new[field])
        ):
    ...
    if rules.get("new_visible_units") and units - previous:
        return "Controller interruption condition matched: new_visible_units " + str(
            sorted(units - previous)
        )
...
def run_dispatch(
    ...
    def finish(outcome, reason, next_action=None, details=None):
        ...
        summary.update(
            ...
            progress={
                "task_index": workflow.get("context", {}).get("task_index", 0),
