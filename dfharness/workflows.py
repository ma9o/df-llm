...
PRIMITIVES = {"move", "wait", "dismiss", "resume", "action_prompt", "key", "click",
              "click_text", "text", "select_unit", "select_option"}
PICKUP_KINDS = {"ENVIRONMENT_PICK_UP_GROUND_ITEM", "ENVIRONMENT_TAKE_ITEM_FROM_CONTAINER",
                "ENVIRONMENT_PICK_UP_BUILDING_ITEM", "ENVIRONMENT_PICK_UP_BUILDING_ITEM_CONTENTS"}
MENU_ACTIONS = {
...
DIRECTIONS = [(0,-1,"n"),(-1,-1,"nw"),(-1,0,"w"),(-1,1,"sw"),
...
def validate_action(action):
    ...
    if kind not in SEMANTIC:
        required, optional = {
            "move": ({"direction"}, set()), "wait": (set(), set()), "dismiss": (set(), set()),
            "resume": (set(), {"dispatch_id"}), "action_prompt": ({"choice"}, set()),
            "key": ({"key"}, set()), "click": ({"x", "y"}, {"button"}),
            "click_text": ({"text"}, set()), "text": ({"text"}, set()),
            "select_unit": ({"unit_id"}, set()), "select_option": ({"option_id"}, set()),
        }[kind]
        ...
        if kind == "move" and action["direction"] not in ("n", "s", "e", "w", "ne", "nw", "se", "sw", "up", "down"):
        ...
        if kind == "text" and (len(action["text"]) > 200 or any(not 32 <= ord(c) <= 126 for c in action["text"])):
...
def next_walk(view, target, constraints):
    ...
    blocked = {(p["x"], p["y"]) for p in constraints.get("blocked_tiles", []) if p["z"] == start["z"]}
    if not constraints.get("allow_occupied", False):
        blocked |= {(u["position"]["x"], u["position"]["y"]) for u in m.get("units", [])
    ...
    for y, row in enumerate(m["walkable"]):
        for x, cell in enumerate(row):
            ...
            if cell == "1" and depth.isdigit() and int(depth) <= constraints.get("max_liquid_depth", 7) and p not in blocked:
                allowed.add(p)
    if goal not in allowed:
        ...
        if depth is not None and (not depth.isdigit() or int(depth) > constraints.get("max_liquid_depth", 7)):
            reasons.append("excluded_liquid_depth")
        return result("needs_input", "Destination is excluded from this route: " + ", ".join(reasons),
                      {"destination": target, "blockers": occupants, "route_exclusions": reasons})
    queue = deque([(start_xy, None)])
    seen = {start_xy}
    while queue:
        p, first = queue.popleft()
        if p == goal:
            dx, dy, direction = first
            return {"input": {"type": "move", "direction": direction},
                    "pending": {"kind": "walk", "destination": {"x": start["x"]+dx, "y": start["y"]+dy, "z": start["z"]}}}
    ...
    return result("needs_input", "No route through the currently observed walkable tiles satisfies the supplied constraints.")
...
def tasks_for(action):
    ...
    if kind in ("equip", "wield"):
        ...
        tasks += [{"kind": "wear" if kind == "equip" else "wield", "item_id": target,
                   "body_part_id": action.get("body_part_id")}]
        disposition = action.get("disposition", "hold")
        if disposition != "hold":
            tasks += [{"kind": disposition, "item_id": i, "container_id": action.get("container_id")}
...
def satisfied(task, view):
    ...
    if kind == "pickup":
        return carried
    if kind == "drop":
        return not carried and item.get("location", {}).get("kind") == "ground" and item["location"].get("container_id") is None
    if kind == "stow":
        return carried and item.get("location", {}).get("container_id") == task["container_id"]
    if kind in ("remove", "wield"):
        return carried and item.get("mode") == "Weapon" and (task.get("body_part_id") is None or item.get("body_part_id") == task["body_part_id"])
    if kind == "wear":
        return carried and item.get("mode") == "Worn" and (task.get("body_part_id") is None or item.get("body_part_id") == task["body_part_id"])
    return False


def menu_signature(view):
    m = view.get("menu") or {}
    return [m.get("kind"), m.get("context"), m.get("scroll"),
            [o["id"] for o in m.get("options", []) if o.get("visible")]]


def close_menu(view):
    return {"input": {"type": "key", "key": "LEAVESCREEN"},
...
def next_step(workflow, view):
    ...
    if kind not in SEMANTIC:
        if kind == "resume" or ctx.get("raw_sent"):
            return result("completed", "Dispatch settled at a game input boundary; inspect the game outcome.")
        ctx["raw_sent"] = True
    ...
    if pending:
        if pending["kind"] == "walk" and view["status"].get("position") != pending["destination"]:
            return result("no_effect", "The movement input did not reach the expected adjacent tile.")
        ...
        if pending["kind"] == "open" and not view.get("menu"):
            return result("needs_input", "The game did not offer the requested inventory/pickup menu.")
        if pending["kind"] == "select" and index < len(tasks) and not satisfied(tasks[index], view):
            if (view.get("menu") or {}).get("choosing_amount"):
                return result("needs_input", "The game requires a pickup quantity; the item has not yet been acquired.")
            ...
            if (tasks[index]["kind"] != "stow" or pending.get("destination")
                    or (view.get("menu") or {}).get("context_item_id") != tasks[index]["item_id"]):
                return result("needs_input", "The selected item action did not satisfy its inventory postcondition.")
    ...
    item = all_items(view).get(task["item_id"])
    if not item:
        return result("needs_input", "Target item is not in the carried or visible nearby item state; it may have moved or the list may be truncated.")
    if task["kind"] == "pickup":
    ...
    if menu:
        ...
        matches = [o for o in menu.get("options", []) if
                   (ctx.get("stow_destination") and o.get("container_id") == target
                    and o.get("kind") == "ENVIRONMENT_PLACE_IN_IT_CONTAINER") or
                   (not ctx.get("stow_destination") and o.get("item_id") == target and o.get("kind") in kinds)]
        if matches:
            if len(matches) != 1:
                return result("needs_input", "Multiple native actions match the item; choose the intended option explicitly.")
            option = matches[0]
            if option.get("visible"):
                return {"input": {"type": "select_option", "option_id": option["id"]},
            ...
            return {"input": {"type": "key", "key": "ADVENTURE_LIST_SCROLL_"+direction},
                    "pending": {"kind": "scroll", "before": menu_signature(view)}}
        if menu.get("kind") == family and (ctx.get("opened_for") == [task["kind"], task["item_id"]] or ctx.get("stow_destination")):
            return result("needs_input", "The game does not offer this item action now; fit, capacity, conflicting equipment, or the current menu may prevent it.")
        return close_menu(view)
    if not view["status"].get("can_move"):
        return result("needs_input", "Return to the default adventure view before starting this item operation.")
