...
def validate(tokens, match, limit, workers):
    ...
    if match not in ("any", "type", "name"):
        raise ValueError("match must be any, type or name")
...
def scan_chunk(sites, tokens, match, limit, origin):
    ...
    needles = [(ALIASES.get(normalized(t), normalized(t)), t.casefold()) for t in tokens]
    for row in sites:
