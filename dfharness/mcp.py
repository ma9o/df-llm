...
INTERRUPT = obj({"blood_loss": {"type": "boolean"}, "new_wounds": {"type": "boolean"},
...
EXECUTION = obj({"mode": {"enum": ["step", "complete"]},
                 "acknowledge": {"type": "boolean"},
                 "max_steps": {"type": "integer", "minimum": 1, "maximum": 64}, "interrupt_on": INTERRUPT})
ACTIONS = [
    obj({"type": {"const": "move"}, "direction": {"enum": ["n", "s", "e", "w", "ne", "nw", "se", "sw", "up", "down"]}}, ("type", "direction")),
    ...
    obj({"type": {"const": "action_prompt"}, "choice": {"enum": ["continue", "stop", "finish"]}}, ("type", "choice")),
    obj({"type": {"const": "key"}, "key": STRING}, ("type", "key")),
    obj({"type": {"const": "select_unit"}, "unit_id": COORD}, ("type", "unit_id")),
    obj({"type": {"const": "click"}, "x": COORD, "y": COORD,
         "button": {"enum": ["left", "right", "middle"]}}, ("type", "x", "y")),
    obj({"type": {"const": "click_text"}, "text": STRING}, ("type", "text")),
    obj({"type": {"const": "text"}, "text": {"type": "string", "minLength": 1, "maxLength": 200}}, ("type", "text")),
]
for name in ("pickup", "remove", "drop"):
    ACTIONS.append(obj({"type": {"const": name}, "item_id": COORD}, ("type", "item_id")))
for name in ("equip", "wield"):
    ACTIONS.append(obj({"type": {"const": name}, "item_id": COORD,
                        "replace": {"type": "array", "items": COORD, "maxItems": 16},
                        "disposition": {"enum": ["hold", "drop", "stow"]},
                        "container_id": COORD, "body_part_id": COORD}, ("type", "item_id")))
ACTIONS += [obj({"type": {"const": "stow"}, "item_id": COORD, "container_id": COORD}, ("type", "item_id", "container_id")),
            obj({"type": {"const": "walk_to"}, "x": COORD, "y": COORD, "z": COORD,
                 "allow_occupied": {"type": "boolean"}, "max_liquid_depth": {"type": "integer", "minimum": 0, "maximum": 7},
                 "blocked_tiles": {"type": "array", "items": POSITION, "maxItems": 500}}, ("type", "x", "y", "z"))]
...
TOOLS = [
    tool("df_status", "Read the complete supported character report and current game/input status, including health, physiology, "
         ...
         "Use df_game_status for lightweight readiness checks. No game input or time advancement.", obj()),
    tool("df_game_status", "Read only game mode, focus, detected modal, player position, active dispatch and adventure input readiness.", obj()),
    tool("df_character_status", "Read the current adventurer's comprehensive character sheet without sending input or advancing game time. "
         ...
         "Includes a state_id guard and current game/input status, without repeating the map or report history.", obj()),
    tool("df_observe", "Read UI text, an ASCII map, health/inventory, visible creatures, full conversation labels, and recent reports with speaker/activity IDs. "
         "Coordinates are zero-based. Map origin + column/row gives world coordinates; UI clicks use separate UI cells.",
         obj({"width": {"type": "integer", "minimum": 1, "maximum": 101},
              "height": {"type": "integer", "minimum": 1, "maximum": 61},
              "center": obj({"x": COORD, "y": COORD, "z": COORD}, ("x", "y", "z")),
              "map": {"type": "boolean"}, "radius": {"type": "integer", "minimum": 0, "maximum": 50}})),
    tool("df_act", "Dispatch a game action under the controller's execution policy and return its outcome, steps, events, and observation. "
         ...
         obj({"action": {"oneOf": ACTIONS}, "expect": STRING, "request_id": STRING, "execution": EXECUTION,
              "timeout": {"type": "number", "minimum": 0.1, "maximum": 300},
              "result_format": {"enum": ["compact", "full"]}}, ("action",)), False),
    tool("df_items", "Read nearby visible ground items and nested container contents in one call, including material, quality, raw mass, wear, armor coverage and weapon definitions.",
         obj({"radius": {"type": "integer", "minimum": 0, "maximum": 50}})),
    tool("df_item", "Inspect one item carried by the adventurer or on visible ground, including its container, inventory role and properties. Fit is checked by the game's wear menu; maker species alone does not establish fit.",
         obj({"item_id": COORD}, ("item_id",))),
    tool("df_interrupt", "Interrupt a dispatch by its request_id. It stops further harness inputs and preserves the recipe for explicit resume; already submitted native actions are not undone. Can run while df_act is executing. df_game_status exposes the active dispatch ID and progress.",
         obj({"dispatch_id": STRING}, ("dispatch_id",)), False),
    tool("df_inspect", "Read a visible world tile, terrain, liquid depth, ground items, and creatures on it.",
         obj({"x": COORD, "y": COORD, "z": COORD}, ("x", "y", "z"))),
    tool("df_keys", "List valid interface_key names from the running game, filtered by substring (e.g. A_INV, A_TALK, SELECT).",
         obj({"filter": STRING})),
    tool("df_wait_ready", "Wait for an already submitted action/turn to finish. Does not advance game time or send inputs.",
...
def validate(value, schema, path="arguments"):
    ...
    if kind == "string" and not schema.get("minLength", 0) <= len(value) <= schema.get("maxLength", float("inf")):
    ...
    def handle(self, message):
        ...
        if method == "initialize":
            ...
            if version not in ("2025-11-25", "2025-06-18", "2025-03-26", "2024-11-05"):
                version = "2025-11-25"
            result = {"protocolVersion": version, "serverInfo": {"name": "df-llm", "version": "0.6.1"},
            ...
            try:
                ...
                if name == "df_keys":
                    ...
                    method_name = {"df_status": "status", "df_game_status": "game_status", "df_character_status": "character_status", "df_observe": "observe", "df_act": "act",
                                   "df_inspect": "inspect", "df_wait_ready": "wait_ready", "df_items": "items",
                                   "df_item": "item", "df_interrupt": "interrupt"}[name]
                    value = getattr(self.client, method_name)(**arguments)
                result = {"content": [{"type": "text", "text": json.dumps(value, ensure_ascii=False)}], "isError": False}
            except Exception as exc:
                error = str(exc)
                if isinstance(exc, DispatchError):
                    error = json.dumps({"error": error, "dispatch_id": exc.dispatch_id,
                    ...
                    if type(cancelled_id) in (str, int):
                        ...
                        if dispatch_id:
                            try:
                                ...
                                print(f"df-llm: cancellation could not interrupt {dispatch_id}: {exc}", file=sys.stderr)
                    continue
                params = message.get("params", {}) if isinstance(message, dict) else {}
                if (isinstance(message, dict) and message.get("method") == "tools/call"
                        and isinstance(params, dict) and params.get("name") == "df_act"
