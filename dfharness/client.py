...
def make_program(request):
    ...
    character_reader = ("assert(load(" + lua_string((ASSETS / "character.lua").read_text()) + "))"
                        if request.get("op") == "character_status" else "nil")
    character_details = ("assert(load(" + lua_string((ASSETS / "character_details.lua").read_text()) + "))"
                         if request.get("op") == "character_status" else "nil")
    character_calculations = ("assert(load(" + lua_string((ASSETS / "character_calculations.lua").read_text()) + "))()"
                              if request.get("op") == "character_status" else "nil")
    return (
        "local glyph=assert(load(" + lua_string((ASSETS / "cp437.lua").read_text()) + "))();"
        "local request=require('json.internal'):new{strictTypes=true}:decode(" + lua_string(request_json) + ");"
        "local result=assert(load(" + lua_string((ASSETS / "bridge.lua").read_text()) + "))(request,glyph," + character_reader + "," + character_details + "," + character_calculations + ");"
...
class Client:
            ...
            with self.log_lock:
                ...
                with self.log_path.open("a", encoding="utf-8") as log:
                    log.write(json.dumps({"at": datetime.now(timezone.utc).isoformat(),
                                          "request": request, "response": envelope,
    ...
    def act(self, action, expect=None, request_id=None, timeout=30, execution=None, result_format="compact"):
...
def render_observation(observation):
    ...
    lines = [f"# {s['df_version']} | DFHack {s['dfhack_version']} | {s['mode']}",
    ...
    if "adventurer" in observation:
        ...
        if player:
            lines.append(f"# Adventurer: {player['name']} | id={player['id']} | position={player['position']}")
    ...
    if observation.get("dispatch"):
        ...
        lines.append(f"# Dispatch: {d['id']} | {d['outcome']} | {len(d['steps'])} steps | {d['reason']}")
        for field, label in (("resume_action", "Resume"), ("next_action", "Next input"), ("details", "Details"),
                             ("prompts", "Handled prompts"), ("prompt_sequence", "Prompt order")):
            if d.get(field):
                lines.append("# " + label + ": " + json.dumps(d[field], ensure_ascii=False))
        for event in d.get("events", []):
            lines.append(f"E{event['id']}|speaker={event['speaker_id']} activity={event['activity_id']}|{event['text']}")
    if observation.get("conversation"):
        lines.append("# Conversation: " + json.dumps(observation["conversation"], ensure_ascii=False))
    ...
    if observation.get("adventurer"):
        lines.append("# Inventory: " + json.dumps(observation["adventurer"].get("inventory", []), ensure_ascii=False))
    if observation.get("nearby_items"):
        lines.append("# Nearby items: " + json.dumps(observation["nearby_items"], ensure_ascii=False))
    if observation.get("menu"):
        lines.append("# Menu: " + json.dumps(observation["menu"], ensure_ascii=False))
    ui = observation["ui"]
    lines.append(f"# UI {ui['width']}x{ui['height']}; U<y>|; x = zero-based Unicode character offset")
    lines += [f"U{row['y']:02d}|{row['text']}" for row in ui["rows"]]
    if observation.get("reports"):
        lines.append("# Recent reports (stable IDs; match activity_id and speaker_id for direct replies):")
        lines += [f"R{r['id']}|speaker={r['speaker_id']} activity={r['activity_id']}|{r['text']}"
                  for r in observation['reports'][-12:]]
    if observation.get("map"):
        m = observation["map"]
        o = m["origin"]
        lines.append(f"# MAP {m['width']}x{m['height']} | origin=({o['x']},{o['y']},{o['z']}) | semantic ASCII")
        lines.append("# World coordinate = origin + (column, row, 0). Map cells are NOT UI click coordinates.")
...
def render_character_status(observation):
    ...
    s = observation["status"]
    lines = [f"# Character status | {s['df_version']} | DFHack {s['dfhack_version']}",
    ...
    if "encumbrance" in c:
        load = c["encumbrance"]
        kg = lambda value: f"{value:.6f}".rstrip("0").rstrip(".")
        ...
        if load.get("weight_complete") and "total_weight_kg" in load:
            ...
            subtotal = kg(load["known_weight_kg"]) + " kg" if "known_weight_kg" in load else "unavailable"
            lines.append(f"Carried weight: unknown; known subtotal {subtotal}")
        burden = load.get("burden", {})
        if burden.get("available"):
            lines.append(f"Burden: {burden['label']} ({burden['capacity_used_percent']:.2f}% of capacity)")
        else:
        ...
        for field, label in (("capacity", "Carrying capacity"), ("load_penalty", "Load penalty")):
            ...
            if not value.get("available"):
                ...
                summary = f"{kg(value['weight_kg'])} kg before movement penalty (skill-adjusted load)"
            else:
                summary = f"+{value['movement_cost_added']} movement cost"
                if "speed_reduction_percent" in value:
                    summary += f"; {value['speed_reduction_percent']:.2f}% lower speed from carried load"
        ...
        if speed is not None:
            lines.append(f"HUD movement: {speed['gait']} {speed['text']} (native displayed speed; not a load penalty)")
        else:
    ...
    if calculated.get("available"):
        lines.append(f"Calculated movement: {calculated['gait']} {calculated['displayed_text']} "
    ...
    for group, attrs in c.get("attributes", {}).items():
        ...
        for a in attrs:
            lines.append(f"{a['name']}: {a.get('value', '?')} / {a.get('effective', '?')} / {a.get('max_value', '?')}")
    if "skills" in c:
        lines.append("# Skills (native records)")
        for skill in c["skills"]:
            lines.append(f"{skill.get('caption', skill['name'])}: {skill.get('rating_name', skill['rating'])} "
