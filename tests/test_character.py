...
def sheet(available=True):
    ...
    result["character"] = {
        "id": 5,
        "name": "Athis Pridesings",
        "position": {"x": 70, "y": 68, "z": 128},
        "identity": {"profession": "Hammerman"},
        "health": {"wounds": 1, "flags": {"unconscious": False}},
        "attributes": {
            "physical": [{"name": "STRENGTH", "value": 1100, "effective": 950, "max_value": 2200}]
        },
        "skills": [
            {
                "name": "HAMMER",
        ...
        "body": {
            "parts": [{"id": 1, "name": "left hand", "active_status_flags": ["missing"]}],
        ...
        "inventory": [
            {
                ...
                "contents": [
                    {
                        "id": 14434,
                        "description": "green jade gem",
                        "location": {"container_id": 495},
                    }
                ],
            }
        ],
        "encumbrance": {
            ...
            "total_weight_kg": 98.46225,
            "known_weight_kg": 98.46225,
...
class CharacterStatusTests(unittest.TestCase):
    ...
    def test_status_cli_returns_full_character_report_and_text_sections(self):
        for text in (False, True):
            with (
                ...
                patch("dfharness.cli.Client") as client,
    ...
    def test_text_preserves_every_section_including_new_and_nested_data(self):
        ...
        result["character"].update(
            {
                "abilities": {"body": [{"name": "Spit", "cooldown_raw": 0}]},
                "relationships": {"family": []},
                "reputation": {"present": False},
    ...
    def test_reading_view_includes_load_contributors_and_displayed_speed(self):
        ...
        for expected in (
            "# Encumbrance",
            "Carried weight: 98.46225 kg",
            "Worn: 98.46225 kg",
            "backpack — 17.51175 kg",
            "HUD movement: Walk 0.471",
            "Carrying capacity: unavailable",
            "Load penalty: unavailable",
        ):
            self.assertIn(expected, text)
    ...
    def test_reading_view_preserves_zero_load_and_zero_displayed_speed(self):
        result = sheet()
        result["character"]["encumbrance"].update(
            total_weight_kg=0, known_weight_kg=0, by_mode=[], heaviest_items=[]
        )
    ...
    def test_reading_summary_includes_calculations_and_need_stages_without_hud(self):
        ...
        c["encumbrance"]["load_penalty"] = {
            "available": True,
            "movement_cost_added": 1123,
            "speed_reduction_percent": 52.896844,
            "excess_weight_kg": 35.42,
        }
        c["movement"] = {
            "effective_speed": {
                "available": True,
                "gait": "Walk",
                "displayed_text": "0.471",
        ...
        text = render_observation(result)
        for expected in (
            "Carrying capacity: 63.04 kg before movement penalty",
            "52.90% lower speed",
            "HUD movement: unavailable",
            "Calculated movement: Walk 0.471",
            "unloaded 1.000",
            "Hunger: No warning (severity 0; counter 0)",
            "next: Hungry at 57600",
            "Thirst: Dehydrated",
            "Sleep: Not required",
        ):
            self.assertIn(expected, text)
