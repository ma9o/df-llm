...
class McpTests(unittest.TestCase):
    def setUp(self):
        ...
        self.init = {"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {"protocolVersion": "2025-11-25"}}
        self.server.handle(self.init)

    def call(self, name, args):
        return self.server.handle({"jsonrpc": "2.0", "id": 3, "method": "tools/call",
    ...
    def test_invalid_mutation_arguments_never_reach_the_game(self):
        for action in ({"type": "move", "direction": "north"},
    ...
    def test_valid_action_and_unknown_tool(self):
        self.assertFalse(self.call("df_act", {"action": {"type": "move", "direction": "w"}})["result"]["isError"])
    ...
    def test_conversation_actions_preserve_state_guard_and_request_id(self):
        for action in ({"type": "select_unit", "unit_id": 4232}, {"type": "dismiss"}):
            args = {"action": action, "expect": "observed-state", "request_id": "one-intended-action"}
    ...
    def test_execution_policy_reaches_dispatch_and_invalid_policies_do_not(self):
        args = {"action": {"type": "resume"}, "execution": {"mode": "complete", "acknowledge": True}, "timeout": 20}
        ...
        for policy in ({"mode": "safe"}, {"risk": "low"}, {"max_steps": 65}, {"acknowledge": 1}):
            self.assertTrue(self.call("df_act", {"action": {"type": "wait"}, "execution": policy})["result"]["isError"])
        self.assertEqual(self.game.calls, [args])
        ...
        for notification in (False, True):
            with self.subTest(notification=notification):
                ...
                call = {"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": {
                        "name": "df_act", "arguments": {"action": {"type": "wait"}, "request_id": "running-action"}}}
                stop = ({"jsonrpc": "2.0", "method": "notifications/cancelled", "params": {"requestId": 3}}
                        if notification else {"jsonrpc": "2.0", "id": 4, "method": "tools/call", "params": {
                            "name": "df_interrupt", "arguments": {"dispatch_id": "running-action"}}})
    ...
    def test_dispatch_error_returns_identity_and_recovery_action(self):
        ...
        self.assertEqual(error["resume_action"], {"type": "resume", "dispatch_id": "recoverable-id"})

    def test_semantic_action_schema_preserves_controller_choices(self):
        args = {"action": {"type": "equip", "item_id": 3, "replace": [2], "disposition": "stow", "container_id": 99},
                "execution": {"mode": "complete", "interrupt_on": {"blood_loss": True, "visible_unit_ids": [20]}},
                "result_format": "compact", "request_id": "equipment-action"}
        self.assertFalse(self.call("df_act", args)["result"]["isError"])
        self.assertEqual(self.game.calls, [args])
