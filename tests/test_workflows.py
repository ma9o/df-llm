...
def item(item_id, mode=None, *, ground=False, container=None, x=1):
    result = {"id": item_id, "description": "identical boot", "type": "SHOES", "quality": 2,
              "location": {"kind": "ground" if ground else "inventory"}}
    if ground:
        result["location"]["position"] = {"x": x, "y": 1, "z": 0}
    if container is not None:
        result["location"]["container_id"] = container
    if mode:
        result.update(mode=mode, body_part_id=15)
        result["location"].update(mode=mode, body_part_id=15)
    return result
...
def menu(kind, ids, *, visible=None, scroll=0, context_item=None, destinations=None):
    ...
    for index, item_id in enumerate(ids):
        o = {"id": f"option:{kind}:{index}:{item_id}", "item_id": item_id, "index": index,
             "label": "identical boot", "kind": kind}
        if visible is None or index in visible:
            o["visible"] = True
        if destinations:
            o["container_id"] = destinations[index]
        options.append(o)
    result = {"kind": "option_list" if kind.startswith("ENVIRONMENT_PICK") else "inventory",
              "context": kind, "scroll": scroll, "options": options}
...
def scene(name, carried=(), ground=(), *, choices=None, x=1, blood=1000, reports=(), units=()):
    return {"state_id": "state-"+name, "effect_id": name,
            "status": {"can_move": choices is None, "ready_for_input": True, "adventurer_id": 1,
                       "position": {"x": x, "y": 1, "z": 0}},
            "adventurer": {"id": 1, "health": {"blood_count": blood, "wounds": 0}, "inventory": list(carried)},
            "nearby_items": list(ground), "menu": choices, "reports": list(reports),
            "ui": {"rows": [{"y": 10, "text": name}]},
            "map": {"origin": {"x": 0, "y": 0, "z": 0}, "walkable": ["11111"]*3,
                    "liquid_depths": ["00000"]*3, "units": list(units)}}
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_pickup_approaches_scrolls_and_selects_by_id_among_identical_labels(self):
        ...
        b = Bridge(scene("before", ground=ground), [scene("arrived", ground=ground, x=2),
                   scene("menu", ground=ground, choices=hidden, x=2), scene("scroll", ground=ground, choices=shown, x=2),
    ...
    def test_step_resume_continues_without_reopening_or_repeating_input(self):
        b = Bridge(scene("before", [item(2, "Weapon")]), [
            scene("menu", [item(2, "Weapon")], choices=menu("DROP_ITEM", [2])), scene("done", ground=[item(2, ground=True)])])
        c = self.client(b)
    ...
    def test_failed_pickup_does_not_complete_or_repeat_selection(self):
        ...
        b = Bridge(scene("before", ground=ground), [scene("menu", ground=ground,
    ...
    def test_equip_replaces_only_named_item_and_disposes_after_verified_wearing(self):
        ...
        b = Bridge(scene("before", [old, other, new]), [
        ...
        r = self.client(b).act({"type": "equip", "item_id": 3, "replace": [2], "disposition": "drop"})
        self.assertEqual(r["dispatch"]["outcome"], "completed")
        selected = [a["option_id"] for a in b.inputs if a["type"] == "select_option"]
        self.assertEqual(selected, ["option:REMOVE_ITEM:1:2", "option:WEAR_ITEM:0:3", "option:DROP_ITEM:0:2"])
        self.assertFalse(any("4" == a.rsplit(":", 1)[-1] for a in selected))
    ...
    def test_unwearable_target_retains_old_gear_and_reports_choices(self):
        ...
        b = Bridge(scene("before", [old, new]), [
            scene("remove-menu", [old, new], choices=menu("REMOVE_ITEM", [2])), scene("removed", [held, new]),
    ...
    def test_stow_uses_container_id_and_verifies_destination(self):
        ...
        b = Bridge(scene("before", carried), [
            scene("put-menu", carried, choices=menu("PUT_ITEM", [2])),
            scene("destinations", carried, choices=menu("ENVIRONMENT_PLACE_IN_IT_CONTAINER", [2, 2],
                  context_item=2, destinations=[98, 99])),
    ...
    def test_wrong_stow_destination_is_not_success_or_automatically_retried(self):
        ...
        b = Bridge(scene("before", carried), [
            scene("put-menu", carried, choices=menu("PUT_ITEM", [2])),
            scene("destinations", carried, choices=menu("ENVIRONMENT_PLACE_IN_IT_CONTAINER", [2], context_item=2, destinations=[99])),
    ...
    def test_completion_mode_does_not_stop_for_damage_unless_controller_requests_it(self):
        for rules, expected in (({}, "completed"), ({"blood_loss": True}, "interrupted")):
            with self.subTest(rules=rules):
                ground = [item(2, ground=True)]
                b = Bridge(scene("before", ground=ground), [
                    scene("menu", ground=ground, choices=menu("ENVIRONMENT_PICK_UP_GROUND_ITEM", [2]), blood=900),
                    scene("done", [item(2)], blood=900)])
                r = self.client(b).act({"type": "pickup", "item_id": 2}, execution={"interrupt_on": rules})
    ...
    def test_caller_visible_unit_condition_interrupts_before_initial_input(self):
        ...
        for action in ({"type": "key", "key": "A_ATTACK"}, {"type": "walk_to", "x": 3, "y": 1, "z": 0}):
    ...
    def test_new_unit_and_report_predicates_are_observable(self):
        for rules, final in (
            ({"new_visible_units": True}, scene("after", units=[{"id": 20, "position": {"x": 2, "y": 1, "z": 0}}])),
            ({"new_wounds": True}, scene("after")),
            ({"report_types": ["COMBAT_STRIKE_DETAILS"]}, scene("after", reports=[{"id": 1, "type": "COMBAT_STRIKE_DETAILS", "text": "strike"}])),
        ):
    ...
    def test_resume_retains_reports_without_retriggering_an_already_reported_interruption(self):
        ...
        c = self.client(b, {"mode": "complete", "interrupt_on": {"report_types": ["COMBAT_STRIKE_DETAILS"]}})
    ...
    def test_lost_input_reply_resumes_from_verified_effect_without_repeating(self):
        ...
        c = self.client(b)
        with self.assertRaisesRegex(DFHackError, "lost response") as error:
            c.act({"type": "drop", "item_id": 2}, request_id="original")
        self.assertEqual(error.exception.dispatch_id, "original")
        self.assertEqual(error.exception.resume_action, {"type": "resume", "dispatch_id": "original"})
    ...
    def test_step_limit_keeps_next_action_and_continuation(self):
        b = Bridge(scene("before", [item(2, "Weapon")]), [scene("menu", [item(2, "Weapon")], choices=menu("DROP_ITEM", [2]))])
    ...
    def test_walk_verifies_position_not_only_changed_ui(self):
        b = Bridge(scene("before"), [scene("blocked", reports=[{"id": 1, "text": "Something blocks you."}])])
    ...
    def test_walking_uses_explicit_route_constraints(self):
        ...
        self.assertEqual(next_walk(v, target, {"blocked_tiles": [target]})["outcome"], "needs_input")
    ...
    def test_compact_deduplicates_help_but_preserves_text_and_encounter_order(self):
        a = {"modal": {"kind": "help", "text": ["Inventory help"]}, "text": ["Inventory help"], "response": {"type": "dismiss"}}
        b = {"modal": {"kind": "announcement"}, "text": ["Different message"], "response": {"type": "dismiss"}}
        v = scene("same")
    ...
    def test_compact_exposes_creature_changes_for_controller_threat_assessment(self):
        ...
        r = self.client(b).act({"type": "wait"}, execution={"interrupt_on": {"new_visible_units": True}})
    ...
    def test_stow_selection_closing_the_menu_returns_unverified_postcondition(self):
        ...
        b = Bridge(scene("before", carried), [scene("menu", carried, choices=menu("PUT_ITEM", [2])),
                                               scene("closed", carried)])
        r = self.client(b).act({"type": "stow", "item_id": 2, "container_id": 99})
        self.assertEqual(r["dispatch"]["outcome"], "needs_input")
        self.assertEqual(len(b.inputs), 2)
    ...
    def test_invalid_item_and_interruption_policies_never_reach_bridge(self):
        c = Client(port=1)
        actions = [{"type": "equip", "item_id": True}, {"type": "equip", "item_id": 2, "replace": [2]},
                   ...
                   {"type": "move", "direction": "north"}, {"type": "wait", "risk": "low"},
                   {"type": "key"}, {"type": "text", "text": "\n"}, {"type": "click", "x": True, "y": 0}]
        with patch.object(c, "request") as request:
            for action in actions:
                with self.assertRaises(ValueError):
                    c.act(action)
            for rules in ({"danger": True}, {"blood_loss": 1}, {"visible_unit_ids": [True]}, {"report_types": "COMBAT"}):
                with self.assertRaises(ValueError):
