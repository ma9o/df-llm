...
def view(effect, modal=None, reports=()):
    ...
    return {"state_id": "state-"+effect, "effect_id": effect, "status": status,
            "reports": list(reports), "ui": {"rows": [{"y": 7, "text": "page "+effect}]}}
...
class DispatchTests(unittest.TestCase):
    def scripted(self, views, *, policy=None, defaults=None, action=None):
        ...
        with patch.object(client, "request", side_effect=bridge):
            result = client.act(action, expect=initial["state_id"], request_id="dispatch-1", execution=policy, result_format="full")
        calls = bridge.calls
    ...
    def test_complete_handles_pages_then_finishes_without_continues(self):
        result, inputs = self.scripted([
            view("help", HELP, [report(1)]), view("page-one", MORE, [report(2)]),
            view("page-two", MORE, [report(3)]), view("last-page", OKAY),
            view("long", LONG), view("done", reports=[report(4)]),
        ], policy={"mode": "complete", "acknowledge": True})
        self.assertEqual([r["action"] for r in inputs[1:]],
    ...
    def test_acknowledgement_and_completion_are_independent(self):
        ...
        self.assertEqual(result["outcome"], "needs_input")
        self.assertEqual(len(inputs), 1)
        result, inputs = self.scripted([view("page", OKAY), view("long", LONG)],
                                      policy={"acknowledge": True})
        self.assertEqual(result["outcome"], "needs_input")
        self.assertEqual(inputs[-1]["action"], {"type": "dismiss"})

    def test_completion_policy_does_not_depend_on_action_risk(self):
        for action in ({"type": "wait"}, {"type": "move", "direction": "n"},
                       {"type": "key", "key": "A_ATTACK"}, {"type": "click", "x": 1, "y": 1}):
            with self.subTest(action=action):
                result, inputs = self.scripted([view("long", LONG), view("done")],
                                              policy={"mode": "complete"}, action=action)
                self.assertEqual(result["outcome"], "completed")
    ...
    def test_per_dispatch_policy_overrides_controller_defaults(self):
        result, inputs = self.scripted([view("help", HELP)],
            defaults={"mode": "complete", "acknowledge": True}, policy={"mode": "step", "acknowledge": False})
        self.assertEqual(result["outcome"], "needs_input")
        self.assertEqual(len(inputs), 1)

    def test_unknown_prompt_returns_even_when_its_button_says_okay(self):
        result, inputs = self.scripted([view("unknown", {"kind": "confirmation", "button": "Okay", "dismissible": True})],
                                      policy={"mode": "complete", "acknowledge": True})
        self.assertEqual(result["outcome"], "needs_input")
        self.assertEqual(len(inputs), 1)
    ...
    def test_step_limit_returns_prompt_and_preserved_events(self):
        result, inputs = self.scripted([view("one", MORE), view("two", MORE, [report(9)])],
    ...
    def test_resume_needs_no_original_game_input(self):
        result, inputs = self.scripted([view("long", LONG), view("done")],
                                      action={"type": "resume"}, policy={"mode": "complete"})
    ...
    def test_duplicate_dispatch_does_not_act_on_new_prompt(self):
        for saved in (None, {"outcome": "completed", "steps": []}):
            ...
            receipt = {"action_id": "old", "duplicate": True, "dispatch": saved, "view": view("new", LONG)}
            with patch.object(client, "request", return_value=receipt) as request:
                result = client.act({"type": "wait"}, request_id="old", execution={"mode": "complete"})
    ...
    def test_timeout_records_submitted_action_and_observed_events(self):
        ...
        with patch.object(client, "request", side_effect=bridge), patch("dfharness.dispatch.time.monotonic", side_effect=[0, 0, 2]):
    ...
    def test_readiness_is_rechecked_when_observation_shows_transition(self):
        ...
        with patch.object(client, "request", side_effect=bridge), patch("dfharness.dispatch.time.sleep"):
    ...
    def test_invalid_policy_is_rejected_before_dispatch(self):
        client = Client(port=1)
        for policy in ({"mode": "safe"}, {"risk": "low"}, {"acknowledge": "yes"},
                       {"max_steps": True}, {"max_steps": 0}, {"max_steps": 65}):
